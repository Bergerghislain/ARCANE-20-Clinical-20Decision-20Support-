...
### IA réelle (Vague 2 — branchement openai_compatible) ✅

- **Provider `openai_compatible` validé end-to-end** : `generate_report` + `argos_respond` (sync et SSE) traversent toute la chaîne `routeur FastAPI → AiService → OpenAiCompatibleClient → POST /v1/chat/completions`.
- **`LLM_JSON_MODE`** (`settings.py`) : `response_format: json_object` désormais optionnel (compat endpoints qui ne le supportent pas).
- **Outillage de validation** :
  - `backend_fastapi/scripts/llm_smoke.py` : smoke end-to-end (report + argos + `--stream`) contre l’endpoint configuré (vLLM réel ou stub).
  - `backend_fastapi/scripts/fake_openai_server.py` : serveur OpenAI-compatible déterministe pour exercer le **vrai chemin réseau** sans GPU.
- **Tests** :
  - `tests/test_ai_router_openai_e2e_integration.py` : end-to-end réel via le routeur (sans DB) — **tourne en CI**.
  - `tests/test_llm_live_integration.py` : test *live* vers un vrai vLLM, **sauté par défaut** (`LLM_LIVE_TEST=1` pour l’activer).
- Détails : `docs/QWEN_INTEGRATION.md` (§6).
